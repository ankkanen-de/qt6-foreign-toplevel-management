...
ForeignToplevelManagerV1::handleFromObject(void *object)
{
	return d->m_toplevels[static_cast<
...
{
	m_toplevels.removeIf(
		[this,
		 toplevel](QMap<struct ::zwlr_foreign_toplevel_handle_v1 *,
...
ForeignToplevelHandleV1::ForeignToplevelHandleV1(
	ForeignToplevelManagerV1Private *manager, void *object)
	: d(new ForeignToplevelHandleV1Private(
		  this, manager,
		  (struct ::zwlr_foreign_toplevel_handle_v1 *)object))
{
}
...
ForeignToplevelHandleV1Private::ForeignToplevelHandleV1Private(
	ForeignToplevelHandleV1 *q, ForeignToplevelManagerV1Private *manager,
	struct ::zwlr_foreign_toplevel_handle_v1 *object)
	: QWaylandClientExtensionTemplate(3)
...
void ForeignToplevelHandleV1Private::zwlr_foreign_toplevel_handle_v1_app_id(
	const QString &appId)
{
	m_appId = appId;
...
void ForeignToplevelHandleV1Private::zwlr_foreign_toplevel_handle_v1_title(
	const QString &title)
{
	m_title = title;
...
void ForeignToplevelHandleV1Private::zwlr_foreign_toplevel_handle_v1_state(
	wl_array *state)
{
	m_state.clear();
	for (ForeignToplevelHandleV1::State *s =
		     (ForeignToplevelHandleV1::State *)state->data;
	     (char *)s < (char *)state->data + state->size; s++) {
		m_state += *s;
	}
...
void ForeignToplevelHandleV1::setState(
	QSet<ForeignToplevelHandleV1::State> state)
{
	...
	foreach(ForeignToplevelHandleV1::State s, state - d->m_state) {
		...
		case ForeignToplevelHandleV1::State::Activated: {
			d->activate(
				static_cast<QtWaylandClient::QWaylandScreen *>(
